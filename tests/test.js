...
test("tracklister.tracklistLooksLikeAList", function(assert) {
  ...
  assert.ok(
    tracklister.tracklistLooksLikeAList(numberedList),
    "It should recognize a numbered tracklist"
  );

  assert.ok(
    tracklister.tracklistLooksLikeAList(numberedListWithLeadingZeroes),
    "It should recognize a numbered tracklist with leading zeroes"
  );

  assert.ok(
    tracklister.tracklistLooksLikeAList(numberedListWithoutDots),
...
test("tracklister.extractArtistAndTrack", function(assert) {
  ...
  const royAyersFixture = {
    artist: "Roy Ayers",
    track: "Running Away"
  };

  assert.deepEquals(
    tracklister.extractArtistAndTrack("Miles Davis - Freddie Freeloader"),
    trackFixture,
  ...
  assert.deepEquals(
    tracklister.extractArtistAndTrack("03. Miles Davis - Freddie Freeloader"),
    trackFixture,
    "It should ignore track numbers"
  );

  assert.deepEquals(
    tracklister.extractArtistAndTrack("3 Miles Davis - Freddie Freeloader"),
    trackFixture,
  ...
  assert.deepEquals(
    tracklister.extractArtistAndTrack("Miles Davis - Freddie Freeloader [Blue Note]"),
    trackFixture,
    "It should strip off anything trailing in brackets"
  );

  assert.deepEquals(
    tracklister.extractArtistAndTrack("Miles Davis - Freddie Freeloader (Blue Note, 1959)"),
    trackFixture,
    "It should strip off anything trailing in parentheses"
  );

  assert.deepEquals(
    tracklister.extractArtistAndTrack("Roy Ayers - Running Away [Strictly Breaks - SB 9802]"),
    royAyersFixture,
  ...
  assert.deepEquals(
    tracklister.extractArtistAndTrack("Sa-Ra Creative Partners - Cosmic Ball"),
    saRaFixture,
    "It should somehow recognize artists containing dashes"
  );
